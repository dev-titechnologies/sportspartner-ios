...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    {
        if(validateOnCharacterChanged)
            [(TextFieldValidator *)textField performSelector:@selector(validate2) withObject:nil afterDelay:0.1];
        else
            [(TextFieldValidator *)textField setRightView:nil];
        if([delegate respondsToSelector:@selector(textField:shouldChangeCharactersInRange:replacementString:)])
            return [delegate textField:textField shouldChangeCharactersInRange:range replacementString:string];
...
-(BOOL)validate1{
    if(isMandatory)
    {
        ...
        {
            NSString *spaces_string =self.text;
            NSString *trimmed_string = [self removeStartSpaceFrom:spaces_string];
            NSLog(@"Trimmed = %@",trimmed_string);
            NSRange whiteSpaceRange = [trimmed_string rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
            if (whiteSpaceRange.location == NSNotFound)
            {
                
                NSLog(@"not Found space after string");
                [self showErrorIconForMsg:@"Enter Your Fullname"];
                return NO;
                
            }
            else
            {
                NSArray* splits = [trimmed_string componentsSeparatedByString: @" "];
                NSString* lastname = [splits objectAtIndex: 1];
                NSString* firstname = [splits objectAtIndex: 0];
                NSLog(@"last :%@",lastname);
...
-(BOOL)validate3{
    if(isMandatory)
    {
        ...
        else
        {
            ...
            NSRange whiteSpaceRange = [spaces_string rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
            if (whiteSpaceRange.location != NSNotFound)
            {
                [self showErrorIconForMsg:@"Enter A Valid String"];
                return NO;
            }
        }
    }
